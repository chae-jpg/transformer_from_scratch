...
if __name__ == "__main__":
    ...
    if num_gpus > 1:
        print(f"GPU {num_gpus}개 감지 → DataParallel 사용")
        model = nn.DataParallel(model)
    else:
        print(f"GPU {num_gpus}개 감지 → 단일 GPU 학습")
    # ──────────────────────────────────────────────────────────
    ...
    for epoch in range(num_epochs):
        ...
        for x, y_in, y_out in tqdm(trainloader):
            ...
            outputs = model(x, y_in, src_mask, tgt_mask)   # (batch, seq_len, vocab)
            outputs = outputs.view(-1, VOCAB_SIZE)          # (batch*seq_len, vocab)
            y_out = y_out.view(-1)
        ...
        if avg_eval_loss < min_loss:
            ...
            # DataParallel의 경우 model.module의 state_dict를 저장해야
            # 나중에 DataParallel 없이도 weights를 불러올 수 있음
            torch.save(get_base_model(model).state_dict(), 'weights.pth')
    ...
    print(f"Training finished. Perplexity: {perplexity:.4f}, BLEU: {bleu:.4f}")
